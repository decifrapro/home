"""Guarda dos atendimentos em SQLite — usado quando há disco próprio.
...
Na Vercel não existe disco que dure; lá vale `app/repositorios/supabase.py`.
Os dois módulos oferecem exatamente as mesmas funções, e `app/db.py` escolhe
qual está valendo.
"""
...
def lease_event(job_id: str, event_id: str, seconds: int = 300) -> bool:
    """Reserva o item para processar. False se outro processo pegou antes."""
    limite = (datetime.now() + timedelta(seconds=seconds)).isoformat()
    agora = datetime.now().isoformat()
    with _lock:
        conn = connect()
        cursor = conn.execute(
            "UPDATE events SET processing_status = ?, leased_until = ?"
            " WHERE job_id = ? AND id = ? AND processing_status IN ('pending', 'failed')"
            " AND (leased_until IS NULL OR leased_until < ?)",
            (ProcessingStatus.PROCESSING.value, limite, job_id, event_id, agora),
        )
        conn.commit()
...
def lease_link(job_id: str, link_id: str, seconds: int = 300) -> bool:
    ...
    with _lock:
        conn = connect()
        cursor = conn.execute(
            "UPDATE links SET status = ?, leased_until = ?"
            " WHERE job_id = ? AND id = ? AND status IN ('pending', 'failed')"
            " AND (leased_until IS NULL OR leased_until < ?)",
            (ProcessingStatus.PROCESSING.value, limite, job_id, link_id, agora),
        )
        conn.commit()
        return cursor.rowcount > 0
...
def release_stale_leases(seconds: int = 900) -> None:
    """Devolve para a fila o que ficou preso (função interrompida no meio)."""
    agora = datetime.now().isoformat()
    with _lock:
        conn = connect()
        conn.execute(
            "UPDATE events SET processing_status = 'pending', leased_until = NULL"
            " WHERE processing_status = 'processing' AND (leased_until IS NULL OR leased_until < ?)",
            (agora,),
        )
        conn.execute(
            "UPDATE links SET status = 'pending', leased_until = NULL"
            " WHERE status = 'processing' AND (leased_until IS NULL OR leased_until < ?)",
            (agora,),
        )
        conn.commit()
